...
class CellMatcher:
    ...
    def __init__(self):
        self._iou_thres = 0.05  # NOTE: This has been hardcoded for the 'ACCURATE' tableformer model..
    ...
    def _translate_bboxes(self, table_bbox, cell_bboxes):
        # cell_bboxes: (N,4) normalized [x1,y1,x2,y2] in table coords (origin TOP-LEFT)
        tbx = np.asarray(table_bbox, dtype=np.float32)
        b = np.asarray(cell_bboxes, dtype=np.float32)
        W = tbx[2] - tbx[0]
        H = tbx[3] - tbx[1]

        # map from table TOP-LEFT normalized to page BOTTOM-LEFT, then to TOP-LEFT
        # Step 1: bottom-left page coords (your original math)
        t_mask = np.array([tbx[0], tbx[3], tbx[0], tbx[3]], dtype=np.float32)
        m = np.array([W, -H, W, -H], dtype=np.float32)
        page_bl = t_mask + m * b  # [x1, y2, x2, y1] in BL
        # Step 2: swap y to TOP-LEFT once, vectorized
        # y_top = page_height - y_bottom; but we want relative within the table box:
        # Equivalent compact re-map:
        # [x1, y1, x2, y2] in TOP-LEFT:
        out = np.empty_like(page_bl)
        out[:, 0] = page_bl[:, 0]
        out[:, 2] = page_bl[:, 2]
        # y1_top = tbx[1] + (tbx[3] - page_bl[:,1])
